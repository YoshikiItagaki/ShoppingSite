					...
					<tr>
						...
						<td>
							<table>
								...
								<tr>
									<c:set var="sum" value="${Math.round(item.product.price * item.count * 1.1) }" />
									<td class="sum">小計（税込み）：${(sum) }円
								</tr>
								<tr>
									<td><a
										href="<%= request.getContextPath() %>/jp.co.aforce.user.CartRemove.execute?product_id=${item.product.product_id}">
											カートから削除 </a></td>
								</tr>
							</table>
						</td>
