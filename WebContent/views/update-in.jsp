...
<body>
	...
	<div class="middle-container">
		...
		<div class="input-container">
			...
			<form
				action="<%=request.getContextPath()%>/jp.co.aforce.user.Update.execute"
				method="post" name="form" class="form"
				onsubmit="return validateUpdateOnSubmission()">
					...
					<table>
						<tr>
							...
							<td><input type="text" name="last_name" class="input-field"
								value="${customer.last_name }" onChange="updateValue"></td>
						</tr>
						<tr>
							<td>氏名：名前</td>
							<td><input type="text" name="first_name" class="input-field"
								value="${customer.first_name }" onChange="updateValue"></td>
						</tr>
						<tr>
							<td></td>
								<td><input type="radio" name="sex" value="1" id="male"> <label for="male">男</label>
								<input type="radio" name="sex" value="2" id="female"><label for="female">女</label>
							</td>
						</tr>
						<tr>
							<td></td>
							<td><select name="birth_year" id="birth_year">
									<option value="${customer.birth_year }">${customer.birth_year }</option>
							</select></td>
						</tr>
						<tr>
							<td></td>
							<td><select name="birth_month" id="birth_month">
									<option value="${customer.birth_month }">${customer.birth_month }</option>
							</select></td>
						</tr>
						<tr>
							<td></td>
							<td><select name="birth_day" id="birth_day">
									<option value="${customer.birth_day}">${customer.birth_day}</option>
							</select></td>
						</tr>
						<tr>
							<td>住所</td>
							<td><input type="text" name="address" class="input-field"
								value="${customer.address }" onChange="validateForm()"></td>
						</tr>
						<tr>
							<td>電話番号</td>
							<td><input type="text" name="phone_number" id="p"
								value="${customer.phone_number }" onchange="validateForm()"></td>
						</tr>
						<tr>
							<td>メールアドレス</td>
							<td><input type="text" name="email"
								value="${customer.email }" onChange=" return updateValue()"></td>
						</tr>
						<tr>
							<td>パスワード</td>
							<td><input type="password" name="password"
								value="${customer.password }" onChange="updateValue()"></td>
						</tr>
						<tr>
						<tr>
							<td></td>
